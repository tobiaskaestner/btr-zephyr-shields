...
This is the **btr-shields clone** (board id ``mikroe_quail_btr``) of the
upstream ``mikroe_quail`` board (zephyr-rigs), adding typed mikroBUS socket
nodes (rigs Convention 4) alongside the untouched legacy ``mikrobus_N_header``
nodes.
...
with the marking on the board. The same four sockets are additionally exposed
as typed ``socket,mikrobus`` nodes (``quail_sock1``..``quail_sock4``) for the
rigs expander.
...
Applications for the ``mikroe_quail_btr`` board can be built and flashed in the usual way

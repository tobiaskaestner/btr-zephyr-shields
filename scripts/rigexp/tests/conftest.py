...
def board_extra_defines(board: str) -> List[str]:
    ...
    of any one board. Passed even when RIG_EXPAND_COMPILE is "rigexp" (the
    default): dts.cmake's own cache variable already defaults to "rigexp",
    so this is a provable no-op on the default path (acceptance criterion
    B) -- simpler than conditioning the define on non-default, and it makes
...
# Banner normalization (rigc-mission-brief.md Sec 3, "the banner finding"):
# 58 emitted goldens carry a provenance banner "generated by rigexp" in
# three comment forms (rig-gen.overlay's /* */, config-sheet.md's <!-- -->,
# context.cmake's #, and rig-gen-includes.dtsi's /* */ -- the SAME /* */
# form as the overlay, so no fourth pattern is needed). That banner is the
# ONLY tool-identity leak into a golden (verified: no argparse usage text
# leaks anywhere), and it is what a from-scratch rigc must be free to name
# itself HONESTLY in ("generated by rigc") without forcing a golden
# rewrite during the differential period. The comparison, not the golden,
# normalizes it back to the frozen spelling -- scoped to exactly this
# token (never a blanket "generated by \\S+", which could over-normalize
# unrelated text that happens to match) and only for the module actually
# under test, read from RIG_EXPAND_COMPILE.
_BANNER_RE = re.compile(r"generated by " + re.escape(RIG_EXPAND_COMPILE))


def _normalize_banner(text: str) -> str:
    """Map 'generated by <RIG_EXPAND_COMPILE>' to the frozen 'generated by
    rigexp' spelling, so the byte-frozen goldens stay usable as the oracle
    for a differing module under test. Provably INERT when RIG_EXPAND_COMPILE
    is the default "rigexp" -- the guard below skips the substitution
    entirely rather than relying on substituting "rigexp" for "rigexp"
    being a no-op, so the default comparison path executes the identical
    code as before this knob existed."""
    if RIG_EXPAND_COMPILE == "rigexp":
        return text
    return _BANNER_RE.sub("generated by rigexp", text)

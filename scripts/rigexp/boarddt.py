...
tests/test_board_read.py guards that this production path agrees with a
direct board_edt call and with pass 2's own edt.pickle.
...
def load_board(name: str, workdir: str, diags: Diagnostics,
    ...
    the expander runs, and dts.cmake computes the recipe itself. Leaving
...
def _discover_board_dts(name: str, diags: Diagnostics) -> Optional[str]:
    ...
    list_boards.find_v2_boards() only attaches a board.yml extend: entry
    to a base it can already see, so a MODULE_ROOT-only scan never learns
    of any of them, and this fallback's own board catalog is consequently
    always empty. The in-build path (dts.cmake) never hits this:
    BOARD_DIR/BOARD_DIRECTORIES are already resolved by boards.cmake

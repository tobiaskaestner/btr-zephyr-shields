...
so rig-gen.overlay stays readable (zephyr,code = <INPUT_KEY_1>;, not a
bare number). Resolving those tokens is a SEPARATE concern from emission,
needed only for the config sheet's human-facing display value; workdir is
threaded into emit() for exactly that (a synthetic cpp/dtlib TU, shared
with the loader's own validation — see dtsio.resolve_token).
"""
...
def emit(solved: Solved, workdir: str,
        include_dirs: list[str] | None = None) -> dict[str, str]:
    """include_dirs is the cpp -I search list threaded into the config
    sheet's own token resolution (dtsio.resolve_token, via _params_table) —
    the same list a caller threads as --include-dir everywhere else; None
    reproduces today's ZEPHYR_INC/MODULE_INC-only search, unchanged."""
    outputs = {
        # rig-gen.*: generated counterparts of the rig folder's own
        # hand-authored <rig-name>.overlay/<rig-name>_defconfig — the
        # -gen infix is what keeps the two distinguishable in a build log
        # (both apply to one build).
        "rig-gen.overlay": _overlay(solved),
        "config-sheet.md": _sheet(solved, workdir, include_dirs),
        "expectations.yml": _expectations(solved),
    }
    # rig-gen-includes.dtsi: only when the rig declares dt-includes: at all
    # — the 12-corpus-rig common case (no parameters, no vocabulary) gets no
    # extra artifact and no include line, zero churn.
    if solved.rig.dt_includes:
        outputs["rig-gen-includes.dtsi"] = _includes_dtsi(solved.rig.dt_includes)
    return outputs


def _includes_dtsi(dt_includes: list[str]) -> str:
    """The fourth generated artifact: nothing but the rig's declared
    #include lines. rig-gen.overlay pulls this in via a QUOTED include
    at the top of the file — quoted-include resolution against the
...
def _instance_extra_props(inst, dev) -> list:
...
def _device_node(s, types, inst, dev, socket, unit=None, reg=None) -> list[str]:
    ...
    for ref in dev.gpio_refs:
        if ref.function == "gpio":
            ...
            if ref.function == "pwm":
                ...
                # (analyzer.py:_collect_channel, category phys-function) —
                # a device with such a ref never earns a solved.channels
                # entry, so cli.py exits on diags.errors before
                # emitter.emit() is ever called (its "cannot fail" contract
                # would otherwise be violated by a raised ValueError here).
                # This assert documents the invariant rather than
                # re-deriving the diagnostic; tripping it means the
                ...
                lines.append(
                    ...
                    f"\t/* {ctype.posname(pos)} */")
    # Every device the analyzer accepted is, by definition,
    ...
    # An SD card on SPI needs its zephyr,sdmmc-disk child node
...
def _sheet(s: Solved, workdir: str, include_dirs: list[str] | None = None) -> str:
    ...
    out += _params_table(s, workdir, include_dirs)
    return "\n".join(out) + "\n"


def _params_table(s: Solved, workdir: str,
        ...
        for dev_label, props in sorted(inst.params.items()):
            for prop, value in sorted(props.items()):
                ...
                if not is_int_literal(value):
                    ...
                    resolved = resolve_token(value, s.rig.dt_includes, workdir, tag,
                                            include_dirs)

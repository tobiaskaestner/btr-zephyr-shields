...
def _cmake_list_escape(value: str) -> str:
    ...
    rig.cmake), so a path containing a literal `;`, `"`, or `\\` survives the
...
def _expand(rig_path: str, shield_dirs: Optional[List[str]], out_dir: str,
    ...
    # cmake/rig.cmake can retrigger configure when any of them changes.
    deps = Depends()
    ...
    # authority on what pass 1 actually read — cmake/rig.cmake appends this

    ...
    def do_add_parser(self, parser_adder):
        ...
        parser.add_argument(
            ...
            help='rig to build: a persisted rig by rig.yml `rig.name`, or a '
                 ...
                 'verbatim as -DRIG=<TARGET>. TARGET may also be a '
                 '`;`-separated LIST of shield elements, each with its own '
                 '@rev and assignments. The trailing assignments apply to '
                 'a PROMOTED SHIELD only -- socket=<label> (which socket '
                 'it plugs, on a board carrying more than one of its '
                 'type), socket.<slot>=<label> (the same, per plug slot, '
                 'for a shield that plugs several at once), '
                 'config.<label>=<value> (which position or address a '
                 'strap or routing jumper is set to) and '
                 '<device>.<prop>=<value> (a shield parameter); they are '
                 'refused on a persisted rig, whose instances each name '
                 'their own. '

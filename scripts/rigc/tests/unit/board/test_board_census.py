...
def test_census_board_two_variants_yield_two_targets_over_one_socket_set() -> None:
    ...
    assert {cb.target for cb in boards} == {
        "mikroe_quail/stm32f427xx/rig", "mikroe_quail/stm32f427yy/rig2"}
    assert len(boards) == 2
...
def test_census_board_collects_sockets_across_several_fragments() -> None:
    boards = census_board(_BOARD_YML_ONE_VARIANT, [
        ("a.dtsi", _FRAGMENT_A), ("b.dtsi", _FRAGMENT_B)])
...
def _device_needing(bus: str) -> Device:
    return Device(name="d", label="d", compatible=None, bus=bus, group=None,
...
def test_boards_for_conforms_via_the_conventional_alias() -> None:
    ...
    board = Board(name="b", sockets={"nucleo_ard": socket},
...
def test_boards_for_inference_zero_candidates_does_not_conform() -> None:
    ...
    verdicts = boards_for(
        rig, {"arduino-r3": _ctype(), "mikrobus": _ctype(name="mikrobus")},
        [_census("t1", board)])
...
def test_boards_for_two_instances_on_a_non_stackable_socket_does_not_conform() -> None:
    ...
    verdicts = boards_for(rig, {"arduino-r3": _ctype(stackable=False)},
                          [_census("t1", board)])

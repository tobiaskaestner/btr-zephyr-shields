"""The loader proper (R2): rig.yml metadata (qualifier axes, board/socket
resolution), the required content file, fragment discovery, and the V1b
delta engine (rigc-r2-brief.md) -- assembled here from the loader's own
submodules:
  ...
  delta.py      -- base topology + the V1b delta engine (the ShieldRef
                   seam)

Everything needing the shield library, board devicetree, or headers
stays a loud Unimplemented refusal. `load()` returns (Rig | None,
diagnostics) rather than raising on a reject: rigc has no accept path
yet, so a load() that finds nothing wrong falls through to cli.py's own
...
from .delta import Topology, apply_delta, parse_instance, parse_wire, union_dt_includes
...
def load(rig_path: str, revision: Optional[str] = None,
        variant: Optional[str] = None,
        ) -> tuple[Optional[Rig], list[Diagnostic]]:
    """Load rig_path (absolute) as far as R2 reaches, returning the built
    Rig (best-effort; None only when nothing further could be attempted
    at all -- no `rig:` block, no name:, or no content file) alongside
    every diagnostic found. rigexp CONTINUES after most errors (rule
    continuation, Sec 6) -- this reproduces that shape rather than
    stopping at the first diagnostic, so a later one is never dropped."""
    doc = parse_marked(rig_path)
    rig_v, diags = require(doc, "rig", "top level")
    if rig_v is None:
        return None, diags
    ...
    # BEFORE any params get token-validated -- deferred wholesale to R3
    # (_check_dt_includes needs cpp/edtlib), so a non-empty result after
    # the union is a loud refusal rather than a silently-skipped check.
    ...
    if rig.dt_includes:
        raise Unimplemented(
            "dt-includes: header preprocessing (needs cpp/edtlib, "
            "deferred to R3)")
    ...
    for item in (insts_v.value if insts_v is not None else []):
        inst, d = parse_instance(item, sock_binding)
        diags += d
        if inst is not None:
    ...
    if variant_delta_v is not None:
        ...
        topology, d = apply_delta(variant_delta_v, "variant", rig.variant,
                                  topology, sock_binding)
        diags += d
    ...
    if revision_delta_v is not None:
        ...
        topology, d = apply_delta(revision_delta_v, "revision", rig.revision,
                                  topology, sock_binding)
        diags += d
